...
/*                                                                           */
/* $FML: ipfilter.c,v 1.15 2002/01/18 10:47:28 fukachan Exp $                */
/*                                                                           */
     ...
     natlookup_t *np;
{
	...
	if (getsockname(fd, (struct sockaddr *)laddrp, &slen) < 0) {
		snprintf(errmsg, sizeof(errmsg), "getsockname failed: %s",
							strerror(errno));
		perror(errmsg);
		return(0);
	}
	...
	if (getpeername(fd, (struct sockaddr *)faddrp, &slen) < 0) {
		snprintf(errmsg, sizeof(errmsg), "getpeername failed: %s",
					strerror(errno));
		perror(errmsg);
		return(0);
	}
